...
export class OrdersService {
    ...
    async createOrder(createOrderDto: CreateOrderDto, owerId: number): Promise<Order> {
        const result = await this.entityManager.transaction( async entityManager => {
            ...
            try {
                const createNewOrder = entityManager.create(Order, createOrderDto)
                createNewOrder.owerId = owerId
                const newOrder = await entityManager.save(createNewOrder)
                for(const item of createOrderDto.orderItems) { 
                    const attributesDefault = await this.attributeDefaultService.getAttributeDefaultById(item.productAttributeDefault)
                    if (attributesDefault.orderNumber + item.number > attributesDefault.inventoryNumber) {
                    ...
                    await entityManager.update(ProductAttributeDefault,attributesDefault.id,attributesDefault)  
                ...
                throw new HttpException( error?.response.message ?? error.sqlMessage, error?.status ?? HttpStatus.INTERNAL_SERVER_ERROR  , { cause: error})
            } 
        })
        return result       
    }
    ...
    async updateOrderByIdFromAdmin(id: number, updateOrderDto: UpdateOrderDto, adminId: number): Promise<Order> {
        if (updateOrderDto.status === OrderStatus.DELIVERING || updateOrderDto.status === OrderStatus.RECEIVED) {
            throw new ForbiddenException("you cannot update order with order status " + updateOrderDto.status)
        }
    ...
    async cancelOrderById(id: number, owerId: number): Promise<Order> {
        ...
        if(currOrder.status !== OrderStatus.PREPARED) {
            throw new ForbiddenException('order not cancel because of status is: ' + currOrder.status)
        }
        ...
        const result = await this.entityManager.transaction(async (entityManager) => {
            ...
            await queryRunner.startTransaction()
            try {
                const cancelOrder = await entityManager.save(Order, currOrder)
                const findOrrderItems = await entityManager.find(OrderItem, { 
                    where: { orderId: { id: currOrder.id}}, 
                    relations: { productAttributeDefault: true}, 
                    select: { productAttributeDefault: { id : true, orderNumber: true}, number: true}
                })
                // hoàn lại số đơn hàng
                for (let item of findOrrderItems) {
                    const attributesDefault = await this.attributeDefaultService.getAttributeDefaultById((item.productAttributeDefault as ProductAttributeDefault).id)
                    attributesDefault.orderNumber = attributesDefault.orderNumber - item.number;
                    await entityManager.save(ProductAttributeDefault, attributesDefault)
                }
                ...
                await queryRunner.rollbackTransaction()
                throw new  HttpException('update  failed', HttpStatus.INTERNAL_SERVER_ERROR, { cause: error})
            }
        })
        return result
    }
    ...
    async updateOrderWithDeliveringStatus(id: number, adminId: number): Promise<Order> {
        const currOrder = await this.orderRepository.findOne({ where: { id: id }})
        if (!currOrder) {
            throw new NotFoundException('not found order with id: ' + id) 
        } 
        ...
        const result = await this.entityManager.transaction(async (entityManager) => {
            ...
            try {
                ...
                const findOrrderItems = await entityManager.find(OrderItem, { 
                    where: { orderId: { id: currOrder.id}}, 
                    relations: { productAttributeDefault: true}, 
                    select: { productAttributeDefault: { id : true, orderNumber: true, productDetailId: true}, number: true}
                })
                for (let item of findOrrderItems) {
                    const attributesDefault = await this.attributeDefaultService.getAttributeDefaultById((item.productAttributeDefault as ProductAttributeDefault).id)
                    attributesDefault.orderNumber = attributesDefault.orderNumber - item.number;
                    attributesDefault.inventoryNumber = attributesDefault.inventoryNumber - item.number
                    await entityManager.save(ProductAttributeDefault, attributesDefault)
                    const product = await entityManager.findOne(Product, { where: { productDetail: { id: (item.productAttributeDefault as ProductAttributeDefault).productDetailId as number}}})
    ...
    async updateOrderWithStatusReceived(id: number, owerId: number): Promise<Order> {
    ...
    async deleteOrderWhenPaymentFailed(id: number, owerId: number): Promise<Order> {
        const currOrder = await this.orderRepository.findOne({ where: { id: id, owerId: { id: owerId}}, })
        if (!currOrder) {
            throw new NotFoundException('you dont have order with id' + id)
        }
        if (currOrder.status !== OrderStatus.PREPARED) {
            throw new ForbiddenException(' you do not delete order with status ' + currOrder.status)
        }
        const result = await this.entityManager.transaction(async (entityManager) => {
            const queryRunner: QueryRunner = entityManager.queryRunner
            await queryRunner.startTransaction()
            try {
                const cancelOrder = await entityManager.remove(Order, currOrder)
                const findOrrderItems = await entityManager.find(OrderItem, { 
                    where: { orderId: { id: currOrder.id}}, 
                    relations: { productAttributeDefault: true}, 
                    select: { productAttributeDefault: { id : true, orderNumber: true}, number: true}
    ...
    async getOrderDetailByIdForUser(id: number, owerId: number): Promise<Order> {
        if(!id || !owerId) {
            throw new BadRequestException(' not found order id or owerId in resquest')
        }
        const order = await this.orderRepository.findOne({ 
            where: {id: id, owerId: {id: owerId}},
            relations: { orderItems: { productAttributeDefault: true}, address: true}
        })
        if(!order) {
