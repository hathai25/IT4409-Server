import { Controller, UseGuards, Get, Req, Post, Body, BadRequestException, Patch, Param, Delete } from '@nestjs/common';
...
export class CategorysController {
    ...
    async createCategory(@Body() createCategory: CreateCategoryDto, @Req() req: any): Promise<ISuccessRespone<CategoryDto>> {
        ...
        const newCategory = await this.categorysService.createCategory(createCategory, req?.user?.adminId )
    ...
    async updateCategory(@Body() updateCategoryDto: UpdateCategoryDto, @Req() req: any, @Param('id') id:number): Promise<ISuccessRespone<CategoryDto>> {
        ...
        const updateCategory = await this.categorysService.updateCategoryById(updateCategoryDto,id, req.user.adminId)
    ...
    async destroyCategoryById(@Param('id') id: number): Promise<ISuccessRespone<CategoryDto>> {
        const destroyCategory = await this.categorysService.destroyCategoryById(id)
    ...
    async softDeleteCategory(@Param('id') id: number, @Req() req: any): Promise<ISuccessRespone<CategoryDto>> {
        ...
        const deleteCategory = await this.categorysService.softDeleteCategoryById(id, req.user.adminId);
