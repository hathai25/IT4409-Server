...
export class ReviewsService {
    ...
    async createReview(createReviewDto: CreateReviewDto, owerId: number): Promise<Review> {
        if (!owerId) {
    ...
    async updateReviewById(updateReviewDto : UpdateReviewDto, id: number, owerId: number): Promise<Review> {
        if (!id || !owerId) {
            throw new BadGatewayException(' not found id or owerId in request');
        }
        const findReview = await this.reviewRepository.findOne({ where: { id: id, owner: { id: owerId}}})
        if (!findReview) {
            throw new NotFoundException('not found review with id '  + id);
        }
        return await this.reviewRepository.save({ ... findReview, ... updateReviewDto})
    } 

    async deleteReviewById(id: number): Promise<Review> {
        const findReview = await this.reviewRepository.findOne({ where: { id: id}})
        if (!findReview) {
            throw new NotFoundException('not found review with id '  + id);
        }
    ...
    async countfindAllReviews(filterReviewDto: FilterReviewDto): Promise<number> {
