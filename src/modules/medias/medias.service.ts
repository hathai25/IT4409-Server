...
export class MediasService {
    ...
    async CreateMedias(createMediaDto: CreateMediaDto): Promise<Media> {
        const media = await this.mediaRepository.findOne({where: {url: createMediaDto.url}})
        if (!media) {
    ...
    async findMediaByUrl(url: string): Promise<Media> {
        const media = await this.mediaRepository.findOne({where: {url: url}})
        if (!media) {
            throw new  NotFoundException('media is exist in synstom ')
        }
        return media
    ...
    async updataMediaById(id: number, updateMediaDto: UpdateMediaDto): Promise<Media> {
       const currMedia = await this.mediaRepository.findOne({where: {id: id}});
       if (!currMedia) {
    ...
    async deleteMediaById(id: number ): Promise<any> {
        try {
            ...
            throw new HttpException('have error to delete media', HttpStatus.BAD_REQUEST, {cause: e})
        }
    }
