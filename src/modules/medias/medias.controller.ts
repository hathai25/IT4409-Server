...
export class MediasController {
    ...
    @Post()
    async createMedia(@Body() createMediaDto: CreateMediaDto): Promise<ISuccessRespone<MediaDto>> {
        const newMedia = await this.mediasService.CreateMedias(createMediaDto)
        return dataToRespone(MediaDto)(newMedia)
    }

    @Patch(':id')
    async updateMediaById(id: number, updateMediaDto: UpdateMediaDto): Promise<ISuccessRespone<MediaDto>> {
        const updateMedia = await this.mediasService.updataMediaById(id, updateMediaDto)
        return dataToRespone(MediaDto)(updateMedia)
    }

    @Delete(':id') 
    async deleteMediaById(@Param('id') id: number): Promise<ISuccessRespone<MediaDto>> {
        const deletMedia = await this.mediasService.deleteMediaById(id);
        return dataToRespone(MediaDto)(deletMedia)
    }

    @Get(':id')
    async getMediaById(@Param('id') id: number): Promise<ISuccessRespone<MediaDto>> {
